    ...
    function view()
    {
        ...
        if ($_GET['id'] != "") {
            ...
            $html .= '<div id="wpfh_main_obit">
									...
									<div class="wpfh_main_obit_info" class="' . $class . '">
										...
										<p><strong>'.__("Service Date","sp-wpfh").':</strong> ' . date("F j, Y", strtotime($r[0]['burial_date'])) . '</p>
										<p>' . stripslashes($r[0]['obit_notes']) . '</p>
										
										</div>
										<div style="clear:both"></div>
									</div>
									<div style="text-align:right">
									' . wpfh_share_fb("http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . "&n=" . $name . "", $name) . '
									</div><h2><a href="'.get_permalink( get_option('wpfh_display_page') ).'?f=obits">'.get_option('wpfh_obit_name_plural').'</a> &raquo; ' . $name . '</h2>';
            ...
            if ($_GET['f'] == 'obit' or $_GET['f'] == '') {
                ...
                $html .= '										
						<p><img src="' . content_url() . '/plugins/wp-funeral-press/thumbs.php?src=' .get_real_image_path ( $r[0]['photo'] ). '&w=200&h=250" style="float:left;margin:5px">' . stripslashes($r[0]['obituary']) . '</p>
						';
            }
