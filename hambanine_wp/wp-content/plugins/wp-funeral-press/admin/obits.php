...
class wpfh_obits_admin{
	...
	function menu(){
		...
		return $wpfh_install->topMenu(''.get_option('wpfh_obit_name_plural').'',$submenu);
	}
...
echo '
  <tr><td></td><td><input type="submit" value="Save" name="save-obit"/></td></tr>
	...
	if($_GET['function'] == 'delete'){
		$wpdb->query("DELETE FROM  ".$wpdb->prefix ."wpfh_obits WHERE id = ".$wpdb->escape($_GET['id'])."	");
			wpfh_redirect('admin.php?page=wpfh');				
	}
		...
		if($_POST['search-obits'] != ""){
		   ...
		   $r = $wpdb->get_results("".$query." ".$pagination->getLimitSql()."", ARRAY_A);
			...
			<form action="" method="post">
				...
				'.__("First Name","sp-wpfh").': <input type="text" name="first_name">   '.__("Last Name","sp-wpfh").': <input type="text" name="last_name"> '.__("Death Date","sp-wpfh").': <input type="text" name="date" class="datepicker">  <input type="submit" name="search-obits" value="Search!">
